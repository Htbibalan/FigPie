...
#### this code suggested by ChatGPT where it tried to enhance some bugs/features in FigPie_v01.ipynb, it takes a different approach for aligning the labels of plots/figures, but for now I'd rather use my own FigPie_v01.ipynb
...
import fitz
from PIL import Image, ImageTk, ImageGrab, ImageDraw, ImageFont, ImageChops, ImageFile
...
class SplashScreen:
    def __init__(self, root, duration=3500):
        self.root = root
        ...
        self.subtitle_label = tk.Label(
            self.root,
            text="Making figures as easy as pie",
...
MAX_ZOOM = 4.0
ZOOM_STEP = 1.15
EXPORT_MAX_PIXELS = 120_000_000
DEFAULT_LABEL_POSITION = "top-left-outside"
...
def estimate_scale_for_export(width_px: int, height_px: int, max_pixels: int = EXPORT_MAX_PIXELS) -> float:
    pixels = width_px * height_px
    if pixels <= max_pixels:
        return 1.0
    return math.sqrt(max_pixels / max(1, pixels))
...
class FigureBoardApp:
    def __init__(self, root: tk.Tk):
        ...
        self.default_label_position = tk.StringVar(value=DEFAULT_LABEL_POSITION)
    ...
    def _build_ui(self):
        ...
        ttk.Button(arrange_box, text="Align left to anchor", command=lambda: self.align_to_anchor("left")).pack(fill="x", padx=6, pady=4)
        ttk.Button(arrange_box, text="Align right to anchor", command=lambda: self.align_to_anchor("right")).pack(fill="x", padx=6, pady=4)
        ttk.Button(arrange_box, text="Align top to anchor", command=lambda: self.align_to_anchor("top")).pack(fill="x", padx=6, pady=4)
        ttk.Button(arrange_box, text="Align bottom to anchor", command=lambda: self.align_to_anchor("bottom")).pack(fill="x", padx=6, pady=4)
        ttk.Button(arrange_box, text="Align vertical centres", command=lambda: self.align_to_anchor("vcenter")).pack(fill="x", padx=6, pady=4)
        ttk.Button(arrange_box, text="Align horizontal centres", command=lambda: self.align_to_anchor("hcenter")).pack(fill="x", padx=6, pady=4)
        ...
        ttk.Spinbox(label_box, from_=8, to=120, textvariable=self.default_label_size, width=8, command=self.apply_default_label_size).pack(anchor="w", padx=6, pady=(0, 6))
        ...
        ttk.Label(label_box, text="Label position").pack(anchor="w", padx=6, pady=(2, 2))
        self.label_position_combo = ttk.Combobox(
            label_box,
            values=["top-left-outside", "top-left-inside", "top-right-inside", "bottom-left-inside", "bottom-right-inside"],
            textvariable=self.default_label_position,
            state="readonly"
        )
        self.label_position_combo.pack(fill="x", padx=6, pady=(0, 6))
        ttk.Button(label_box, text="Apply label position to selected/all", command=self.apply_label_position).pack(fill="x", padx=6, pady=4)
        ttk.Button(label_box, text="Realign labels to selection", command=self.realign_selected_labels).pack(fill="x", padx=6, pady=4)
        ...
        ttk.Combobox(row, values=["px", "in", "cm", "mm"], textvariable=self.canvas_unit, width=6, state="readonly").pack(side="left", padx=(4, 10))
        ...
        ttk.Spinbox(canvas_box, from_=0, to=1000, textvariable=self.gap_x, width=8, command=self.on_gap_change).pack(anchor="w", padx=6, pady=(0, 6))
        ttk.Label(canvas_box, text="Gap Y").pack(anchor="w", padx=6, pady=(2, 2))
        ttk.Spinbox(canvas_box, from_=0, to=1000, textvariable=self.gap_y, width=8, command=self.on_gap_change).pack(anchor="w", padx=6, pady=(0, 6))
        ...
        export_box = ttk.LabelFrame(left, text="Project and export")
        ...
        ttk.Checkbutton(prop_box, text="Show panel label", variable=self.sel_show_label_var).grid(row=13, column=0, columnspan=2, sticky="w", padx=6, pady=4)
        ttk.Button(prop_box, text="Apply changes", command=self.apply_selected_properties).grid(row=14, column=0, columnspan=2, sticky="ew", padx=6, pady=6)
        ttk.Button(prop_box, text="Rich text editor", command=self.open_rich_text_editor).grid(row=15, column=0, columnspan=2, sticky="ew", padx=6, pady=4)
        ttk.Button(prop_box, text="Bring to front", command=self.bring_to_front).grid(row=16, column=0, columnspan=2, sticky="ew", padx=6, pady=4)
        ttk.Button(prop_box, text="Send to back", command=self.send_to_back).grid(row=17, column=0, columnspan=2, sticky="ew", padx=6, pady=4)
        ttk.Button(prop_box, text="Pick text colour", command=self.pick_selected_text_colour).grid(row=18, column=0, columnspan=2, sticky="ew", padx=6, pady=4)
    ...
    def _build_context_menu(self):
        ...
        self.context_menu.add_separator()
        self.context_menu.add_command(label="Toggle erase mode", command=self.toggle_erase_mode)
    ...
    def zoom_by(self, factor: float):
        ...
        relx = x0 / max(1, self.sx(self.canvas_w))
        rely = y0 / max(1, self.sy(self.canvas_h))
        ...
        for item in items:
            if isinstance(item, PanelItem):
                ...
                cloned.append(TextItem(
                    ...
                    fill=item.fill, align=item.align,
                    background=item.background, outline=item.outline,
                    rotation=item.rotation, line_spacing=item.line_spacing,
                    z_index=item.z_index, group_id=item.group_id,
                ))
        return cloned
    ...
    def add_files(self):
        filetypes = [("Supported", "*.png *.jpg *.jpeg *.tif *.tiff *.bmp *.gif *.pdf"), ("Images", "*.png *.jpg *.jpeg *.tif *.tiff *.bmp *.gif"), ("PDF files", "*.pdf"), ("All files", "*.*")]
    ...
    def label_offsets_for_position(self, panel: PanelItem, pos: Optional[str] = None) -> Tuple[int, int]:
        pos = pos or panel.label_position or DEFAULT_LABEL_POSITION
        font = get_font(panel.label_font_family, max(6, panel.label_font_size), bold=True)
        tmp = Image.new("RGBA", (10, 10), (255, 255, 255, 0))
        draw = ImageDraw.Draw(tmp)
        bbox = draw.textbbox((0, 0), panel.label or "A", font=font)
        tw = bbox[2] - bbox[0]
        th = bbox[3] - bbox[1]
        pad = 8
        if pos == "top-left-inside":
            return pad, pad
        if pos == "top-right-inside":
            return max(0, panel.w - tw - pad), pad
        if pos == "bottom-left-inside":
            return pad, max(0, panel.h - th - pad)
        if pos == "bottom-right-inside":
            return max(0, panel.w - tw - pad), max(0, panel.h - th - pad)
        return pad, -th - 6
    ...
    def _add_panel_from_pil(self, img: Image.Image, source_path: Optional[str] = None):
        ...
        panel = PanelItem(
            id=self.get_next_id(), source_path=source_path, pil_image=img, original_size=(ow, oh),
            x=50 + offset, y=50 + offset, w=w, h=h, label="", show_label=True,
            label_font_size=self.default_label_size.get(), label_font_family=self.default_font_family.get(),
            z_index=max((i.z_index for i in self.items), default=-1) + 1,
            label_position=self.default_label_position.get(),
        )
        panel.label_offset_x, panel.label_offset_y = self.label_offsets_for_position(panel)
    ...
    def add_text_box(self, default_text="Text"):
        self.save_undo_state()
        item = TextItem(
            id=self.get_next_id(), text=default_text, rich_runs=make_plain_runs(default_text),
            x=60 + 15 * (len(self.items) % 10), y=60 + 15 * (len(self.items) % 10),
            w=320, h=90, font_size=self.default_text_size.get(), font_family=self.default_font_family.get(),
            line_spacing=DEFAULT_LINE_SPACING, z_index=max((i.z_index for i in self.items), default=-1) + 1,
        )
    ...
    def add_caption_below_last_figure(self):
        ...
        caption = TextItem(
            id=self.get_next_id(), text="Caption", rich_runs=make_plain_runs("Caption"),
            x=x1, y=y2 + self.gap_y.get(), w=max(300, x2 - x1), h=120,
            font_size=self.default_text_size.get(), font_family=self.default_font_family.get(),
            line_spacing=DEFAULT_LINE_SPACING, z_index=max((i.z_index for i in self.items), default=-1) + 1,
        )
    ...
    def redraw(self):
        ...
        if self.marquee_start and self.marquee_current:
            ...
            self.canvas.create_rectangle(self.sx(x1), self.sy(y1), self.sx(x2), self.sy(y2), outline="#2a73ff", dash=(5, 3), width=2)
    ...
    def _draw_panel(self, item: PanelItem):
        ...
        if item.show_label and item.label:
            ...
            self.canvas.create_text(tx, ty, text=item.label, anchor="nw", font=font_style, fill="black")
        ...
        for ch in chars:
            ...
            if current_line and current_width + cw > max_width:
                if last_space_idx >= 0:
                    ...
                    last_space_idx = -1
                    for idx, cc in enumerate(current_line):
                        if cc["text"] == " ":
                            last_space_idx = idx
                else:
                    lines.append(self._compress_line_segments(draw, current_line, font_family, font_size))
        ...
        for line_idx, line in enumerate(lines):
            ...
            if item.align == "center":
                ...
                for seg in line:
                    ...
                    x += seg["width"]
                    max_h = max(max_h, seg["height"])
            y += max_h + max(2, int(round(4 * scale))) + int(round(max_h * extra_factor))
    ...
    def get_selected_items(self) -> List[CanvasItem]:
        return [item for item_id in self.selected_ids if (item := self.get_item_by_id(item_id)) is not None]
    ...
    def on_canvas_motion(self, event):
        ...
        if item is not None:
            if isinstance(item, PanelItem) and self.point_hits_panel_label(item, x, y):
                cursor = "fleur"
            elif item.resize_handle_hit(x, y, self.handle_model_size()):
                cursor = "sizing"
            else:
                cursor = "fleur"
        if self.erase_mode:
    ...
    def on_canvas_press(self, event):
        ...
        clicked_ids = [item.id]
        if item.group_id is not None and not ctrl:
    ...
    def on_canvas_drag(self, event):
        ...
        if self.erase_mode:
            if self.erase_start:
                ...
                self.redraw()
            return
        if self.marquee_start is not None and self.drag_mode is None:
            self.marquee_current = (x, y)
            self.update_marquee_selection(live=True)
            self.redraw()
            return
        ...
        if self.drag_mode == "move":
            ...
            item = self.get_item_by_id(item_id)
            if item is not None:
                _, _, ow, oh = self.drag_origin_map[item_id]
    ...
    def update_marquee_selection(self, live=False):
        if self.marquee_start is None or self.marquee_current is None:
            return
        x1 = min(self.marquee_start[0], self.marquee_current[0])
        y1 = min(self.marquee_start[1], self.marquee_current[1])
        x2 = max(self.marquee_start[0], self.marquee_current[0])
        y2 = max(self.marquee_start[1], self.marquee_current[1])
        rect = (x1, y1, x2, y2)
        found = []
        for item in self.items:
            bx1, by1, bx2, by2 = item.bbox()
            if rects_overlap(rect, (bx1, by1, bx2, by2), pad=0):
                found.append(item.id)
        if self.marquee_additive:
            base = set(self.selected_ids)
            base.update(found)
            self.selected_ids = [i.id for i in self.items if i.id in base]
        else:
            self.selected_ids = found
        self.anchor_selected_id = self.selected_ids[-1] if self.selected_ids else None
        self.refresh_selected_panel(live=live)

    def on_canvas_release(self, event):
        x, y = self.model_from_event(event)
        if self.erase_mode and self.erase_start:
            self.apply_erase_rectangle(self.erase_start[0], self.erase_start[1], x, y)
            self.erase_start = None
            self.erase_current = None
            self.redraw()
            return
        if self.marquee_start is not None and self.drag_mode is None:
            self.marquee_current = (x, y)
            self.update_marquee_selection(live=False)
            if self.before_drag_state is not None and self.state_changed(self.before_drag_state, self.snapshot_state()):
    ...
    def state_changed(self, a, b) -> bool:
        ...
        for ia, ib in zip(a["items"], b["items"]):
            ...
            if isinstance(ia, PanelItem):
                if (ia.x, ia.y, ia.w, ia.h, ia.group_id, ia.z_index, ia.label, ia.show_label, ia.label_offset_x, ia.label_offset_y, ia.label_position) != \
                   (ib.x, ib.y, ib.w, ib.h, ib.group_id, ib.z_index, ib.label, ib.show_label, ib.label_offset_x, ib.label_offset_y, ib.label_position):
                    return True
            else:
                if (ia.x, ia.y, ia.w, ia.h, ia.group_id, ia.z_index, ia.text, ia.font_size, ia.font_family, ia.align, ia.rotation, ia.fill, ia.bold, ia.italic, ia.line_spacing, ia.rich_runs) != \
                   (ib.x, ib.y, ib.w, ib.h, ib.group_id, ib.z_index, ib.text, ib.font_size, ib.font_family, ib.align, ib.rotation, ib.fill, ib.bold, ib.italic, ib.line_spacing, ib.rich_runs):
                    return True
        return False
    ...
    def on_double_click(self, event):
        ...
        if isinstance(item, PanelItem):
            ...
            if new_label is not None:
                ...
                item.label_offset_x, item.label_offset_y = self.label_offsets_for_position(item)
                self.refresh_selected_panel()
                self.redraw()
    ...
    def duplicate_selected(self):
        ...
        for sel in selected:
            if isinstance(sel, PanelItem):
                dup = PanelItem(id=self.get_next_id(), source_path=sel.source_path, pil_image=sel.pil_image.copy() if sel.pil_image else None, original_size=sel.original_size, x=sel.x + 20, y=sel.y + 20, w=sel.w, h=sel.h, label=sel.label, show_label=sel.show_label, label_font_size=sel.label_font_size, label_font_family=sel.label_font_family, label_offset_x=sel.label_offset_x, label_offset_y=sel.label_offset_y, border_width=sel.border_width, border_color=sel.border_color, z_index=max((i.z_index for i in self.items), default=0) + 1, group_id=sel.group_id, label_position=sel.label_position)
            else:
                dup = TextItem(id=self.get_next_id(), text=sel.text, x=sel.x + 20, y=sel.y + 20, w=sel.w, h=sel.h, font_size=sel.font_size, font_family=sel.font_family, bold=sel.bold, italic=sel.italic, rich_runs=json.loads(json.dumps(sel.rich_runs)), fill=sel.fill, align=sel.align, background=sel.background, outline=sel.outline, rotation=sel.rotation, line_spacing=sel.line_spacing, z_index=max((i.z_index for i in self.items), default=0) + 1, group_id=sel.group_id)
    ...
    def regenerate_labels(self, silent=False):
        panels = [i for i in self.items if isinstance(i, PanelItem)]
        panels = sorted(panels, key=lambda p: p.id if self.keep_label_order.get() else (p.y, p.x))
        for idx, panel in enumerate(panels):
            ...
            panel.label_position = self.default_label_position.get()
            panel.label_offset_x, panel.label_offset_y = self.label_offsets_for_position(panel)
        self.redraw()
        if not silent:
    ...
    def apply_label_position(self):
        targets = [i for i in self.get_selected_items() if isinstance(i, PanelItem)]
        if not targets:
            targets = [i for i in self.items if isinstance(i, PanelItem)]
        if not targets:
            return
        self.save_undo_state()
        pos = self.default_label_position.get()
        for panel in targets:
            panel.label_position = pos
            panel.label_offset_x, panel.label_offset_y = self.label_offsets_for_position(panel, pos)
        self.redraw()
        self.set_status(f"Applied label position: {pos}")

    def realign_selected_labels(self):
        targets = [i for i in self.get_selected_items() if isinstance(i, PanelItem)]
        if not targets:
            return
        self.save_undo_state()
        for panel in targets:
            panel.label_offset_x, panel.label_offset_y = self.label_offsets_for_position(panel, panel.label_position)
        self.redraw()
        self.set_status("Realigned selected labels")
    ...
    def get_selection_units(self) -> List[List[CanvasItem]]:
        ...
        for item in selected:
            ...
            if item.group_id is not None:
                members = [m for m in self.items if m.group_id == item.group_id and m.id in self.selected_ids]
                units.append(sorted(members, key=lambda a: a.id))
    ...
    def grid_layout(self, panels: List[PanelItem], rows: int, cols: int):
        ...
        gap_x = max(0, self.gap_x.get())
        gap_y = max(0, self.gap_y.get())
        ...
        for idx, p in enumerate(panels_sorted):
            ...
            p.w = new_w
            p.h = new_h
            p.label_offset_x, p.label_offset_y = self.label_offsets_for_position(p, p.label_position)
            p._preview_cache_key = None
            p._preview_cache_photo = None

    def _distribution_reference_units(self, axis: str) -> List[List[CanvasItem]]:
        units = self.get_selection_units()
        if len(units) >= 2:
            return units
        panels = [i for i in self.items if isinstance(i, PanelItem)]
        if len(panels) >= 2:
            return [[p] for p in sorted(panels, key=lambda z: z.x if axis == "x" else z.y)]
        return []

    def distribute_h(self):
        units = self._distribution_reference_units("x")
        if len(units) < 2:
            return
        self.save_undo_state()
        units = sorted(units, key=lambda u: self.unit_bbox(u)[0])
        top_y = min(self.unit_bbox(u)[1] for u in units)
        left = min(self.unit_bbox(u)[0] for u in units)
        cur_x = left
        gap = max(0, self.gap_x.get())
        for u in units:
            self.move_unit_to(u, new_x=cur_x, new_y=top_y)
    ...
    def distribute_v(self):
        units = self._distribution_reference_units("y")
        if len(units) < 2:
            return
        self.save_undo_state()
        units = sorted(units, key=lambda u: self.unit_bbox(u)[1])
        left_x = min(self.unit_bbox(u)[0] for u in units)
        top = min(self.unit_bbox(u)[1] for u in units)
        cur_y = top
        gap = max(0, self.gap_y.get())
        for u in units:
            self.move_unit_to(u, new_x=left_x, new_y=cur_y)
    ...
    def align_to_anchor(self, where: str):
        ...
        self.redraw()
        self.set_status(f"Aligned to anchor: {where}")
            ...
            for i in range(len(panels)):
                for j in range(i + 1, len(panels)):
                    ...
                    if rects_overlap(a.bbox(), b.bbox(), pad=0):
                        ...
                        changed = True
        self.redraw()
        self.set_status("Resolved overlaps")
    ...
    def render_final_image(self, dpi: int) -> Tuple[Image.Image, Optional[int]]:
        req_scale = max(1.0, dpi / 100.0)
        out_w = max(1, int(round(self.canvas_w * req_scale)))
        out_h = max(1, int(round(self.canvas_h * req_scale)))
        safety_scale = estimate_scale_for_export(out_w, out_h, EXPORT_MAX_PIXELS)
        actual_scale = req_scale * safety_scale
        actual_dpi = max(72, int(round(dpi * safety_scale)))
        out_w = max(1, int(round(self.canvas_w * actual_scale)))
        out_h = max(1, int(round(self.canvas_h * actual_scale)))
        ...
        for item in sorted(self.items, key=lambda x: x.z_index):
            if isinstance(item, PanelItem):
                if item.pil_image is None:
                    continue
                ix = int(round(item.x * actual_scale))
                iy = int(round(item.y * actual_scale))
                iw = max(1, int(round(item.w * actual_scale)))
                ih = max(1, int(round(item.h * actual_scale)))
                resized = item.pil_image.resize((iw, ih), Image.LANCZOS).convert("RGB")
                out.paste(resized, (ix, iy))
                if item.border_width > 0:
                    draw.rectangle([ix, iy, ix + iw, iy + ih], outline=item.border_color, width=max(1, int(round(item.border_width * actual_scale))))
                if item.show_label and item.label:
                    font = get_font(item.label_font_family, max(6, int(round(item.label_font_size * actual_scale))), bold=True)
                    draw.text((int(round((item.x + item.label_offset_x) * actual_scale)), int(round((item.y + item.label_offset_y) * actual_scale))), item.label, fill="black", font=font)
            else:
                ix = int(round(item.x * actual_scale))
                iy = int(round(item.y * actual_scale))
                txt_img = self._build_textbox_image(item, scale=actual_scale)
                out.paste(txt_img.convert("RGB"), (ix, iy), txt_img)
        if self.auto_trim_on_export.get():
            out = self.trim_image(out, pad=max(0, int(round(self.outer_margin.get() * actual_scale))))
        return out, (actual_dpi if actual_dpi != dpi else None)
    ...
    def export_image(self, fmt: str):
        ...
        try:
            ...
            img, adjusted_dpi = self.render_final_image(dpi=dpi)
            final_dpi = adjusted_dpi or dpi
            save_kwargs = {"dpi": (final_dpi, final_dpi)}
            if fmt == "TIFF":
                save_kwargs["compression"] = "tiff_deflate"
            img.save(path, format=fmt, **save_kwargs)
            msg = f"Exported {fmt}: {path}"
            if adjusted_dpi:
                msg += f" (auto-limited to {adjusted_dpi} DPI to keep export size safe)"
            self.set_status(msg)
        except Exception as e:
    ...
    def export_pdf(self):
        ...
        try:
            ...
            final_dpi = adjusted_dpi or dpi
            bio = io.BytesIO()
            img.save(bio, format="PNG", dpi=(final_dpi, final_dpi))
            png_bytes = bio.getvalue()
            width_pt = img.width / final_dpi * 72.0
            height_pt = img.height / final_dpi * 72.0
            doc = fitz.open()
            page = doc.new_page(width=width_pt, height=height_pt)
            page.insert_image(fitz.Rect(0, 0, width_pt, height_pt), stream=png_bytes)
            doc.save(path, deflate=True, garbage=4)
            doc.close()
            msg = f"Exported PDF: {path}"
            if adjusted_dpi:
                msg += f" (auto-limited to {adjusted_dpi} DPI to keep export size safe)"
            self.set_status(msg)
        except Exception as e:
    ...
    def item_to_project_dict(self, item: CanvasItem) -> Dict:
        if isinstance(item, PanelItem):
            return {"kind": "panel", "id": item.id, "source_path": item.source_path, "image_b64": image_to_base64_png(item.pil_image) if item.pil_image else None, "original_size": list(item.original_size), "x": item.x, "y": item.y, "w": item.w, "h": item.h, "label": item.label, "show_label": item.show_label, "label_font_size": item.label_font_size, "label_font_family": item.label_font_family, "label_offset_x": item.label_offset_x, "label_offset_y": item.label_offset_y, "border_width": item.border_width, "border_color": item.border_color, "z_index": item.z_index, "group_id": item.group_id, "label_position": item.label_position}
        return {"kind": "text", "id": item.id, "text": item.text, "rich_runs": item.rich_runs, "x": item.x, "y": item.y, "w": item.w, "h": item.h, "font_size": item.font_size, "font_family": item.font_family, "bold": item.bold, "italic": item.italic, "fill": item.fill, "align": item.align, "background": item.background, "outline": item.outline, "rotation": item.rotation, "line_spacing": item.line_spacing, "z_index": item.z_index, "group_id": item.group_id}

    def project_dict_to_item(self, data: Dict) -> CanvasItem:
        if data["kind"] == "panel":
            img = base64_png_to_image(data["image_b64"]) if data.get("image_b64") else None
            return PanelItem(id=data["id"], source_path=data.get("source_path"), pil_image=img, original_size=tuple(data.get("original_size", img.size if img else (100, 100))), x=data["x"], y=data["y"], w=data["w"], h=data["h"], label=data.get("label", ""), show_label=data.get("show_label", True), label_font_size=data.get("label_font_size", DEFAULT_LABEL_FONT_SIZE), label_font_family=data.get("label_font_family", "Arial"), label_offset_x=data.get("label_offset_x", 10), label_offset_y=data.get("label_offset_y", -34), border_width=data.get("border_width", 0), border_color=data.get("border_color", "black"), z_index=data.get("z_index", 0), group_id=data.get("group_id"), label_position=data.get("label_position", DEFAULT_LABEL_POSITION))
        return TextItem(id=data["id"], text=data.get("text", ""), rich_runs=data.get("rich_runs", make_plain_runs(data.get("text", ""))), x=data["x"], y=data["y"], w=data["w"], h=data["h"], font_size=data.get("font_size", DEFAULT_TEXT_FONT_SIZE), font_family=data.get("font_family", "Arial"), bold=data.get("bold", False), italic=data.get("italic", False), fill=data.get("fill", "black"), align=data.get("align", "left"), background=data.get("background"), outline=data.get("outline"), rotation=data.get("rotation", 0), line_spacing=data.get("line_spacing", DEFAULT_LINE_SPACING), z_index=data.get("z_index", 0), group_id=data.get("group_id"))
    ...
    def save_project(self):
        ...
        if not path:
            return
        try:
            payload = {"version": PROJECT_VERSION, "app": APP_TITLE, "canvas_w": self.canvas_w, "canvas_h": self.canvas_h, "bg_color": self.bg_color, "outer_margin": self.outer_margin.get(), "gap_x": self.gap_x.get(), "gap_y": self.gap_y.get(), "default_label_size": self.default_label_size.get(), "default_text_size": self.default_text_size.get(), "default_font_family": self.default_font_family.get(), "default_label_position": self.default_label_position.get(), "keep_label_order": self.keep_label_order.get(), "show_grid": self.show_grid.get(), "view_zoom": self.view_zoom, "next_id": self.next_id, "next_group_id": self.next_group_id, "items": [self.item_to_project_dict(i) for i in self.items]}
    ...
    def open_project(self):
        ...
        try:
            ...
            self.default_label_position.set(payload.get("default_label_position", DEFAULT_LABEL_POSITION))
    ...
    def on_canvas_ctrl_mousewheel(self, event):
        self.zoom_by(ZOOM_STEP if event.delta > 0 else 1 / ZOOM_STEP)
...
def main():
    ...
    splash_root = tk.Tk()
    SplashScreen(splash_root, duration=3500)
